...
const STATUS_STYLE: Record<string, { iconBg: string; iconColor: string; badgeColor: string }> = {
  success:  { iconBg: 'bg-emerald-500/10', iconColor: 'text-emerald-500', badgeColor: 'text-emerald-500 bg-emerald-500/10 border border-emerald-500/20' },
  failed:   { iconBg: 'bg-red-500/10',     iconColor: 'text-red-500',     badgeColor: 'text-red-500 bg-red-500/10 border border-red-500/20' },
  pending:  { iconBg: 'bg-amber-500/10',   iconColor: 'text-amber-500',   badgeColor: 'text-amber-500 bg-amber-500/10 border border-amber-500/20' },
};
const TYPE_STYLE: Record<string, { iconBg: string; iconColor: string }> = {
  training: { iconBg: 'bg-violet-500/10', iconColor: 'text-violet-500' },
  report:   { iconBg: 'bg-blue-500/10',   iconColor: 'text-blue-500' },
  analysis: { iconBg: 'bg-purple-500/10', iconColor: 'text-purple-500' },
  alert:    { iconBg: 'bg-amber-500/10',  iconColor: 'text-amber-500' },
  capa:     { iconBg: 'bg-rose-500/10',   iconColor: 'text-rose-500' },
  sensor:   { iconBg: 'bg-cyan-500/10',   iconColor: 'text-cyan-500' },
};
...
export const SafetyAutoPilot: React.FC<Props> = () => {
  ...
  return (
    <SMCard className="rounded-2xl overflow-hidden flex flex-col">
      <div className="relative p-5 flex items-center justify-between bg-surface-sunken border-b border-surface-border overflow-hidden">
        ...
        <div className="flex items-center gap-3 relative">
          ...
          <div>
            <h3 className="text-sm font-bold text-text-primary tracking-tight">Safety Auto-Pilot</h3>
        ...
        <button
          ...
          disabled={isScanning}
          className="p-2 rounded-xl hover:bg-surface-overlay text-text-muted hover:text-text-primary transition-all disabled:opacity-50"
      ...
      {/* Timeline */}
      <div className="flex-1 overflow-y-auto p-4 space-y-1 bg-surface-raised">
        {isLoading ? (
          Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="flex gap-3 py-3">
              <div className="w-9 h-9 rounded-xl bg-surface-sunken animate-pulse shrink-0" />
              <div className="flex-1 space-y-2 pt-1">
                <div className="h-3 w-2/3 bg-surface-sunken rounded animate-pulse" />
                <div className="h-2 w-full bg-surface-sunken rounded animate-pulse" />
                <div className="h-2 w-4/5 bg-surface-sunken rounded animate-pulse" />
              </div>
            </div>
          ))
        ) : (
            ...
            {actions.map((action, i) => (
              ...
              >
                ...
                <div className="flex flex-col items-center shrink-0">
                  ...
                  {i < actions.length - 1 && (
                    <div className="w-px flex-1 bg-surface-border my-1.5" />
                  )}
                </div>
                {/* Content */}
                <div className="flex-1 pb-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <h4 className="text-xs font-bold text-text-primary leading-tight">{action.title}</h4>
                  ...
                  <p className="text-[11px] text-text-secondary leading-relaxed mb-2">{action.description}</p>
      ...
      {/* Footer */}
      <div className="p-4 bg-surface-sunken border-t border-surface-border">
        <SMButton
          variant="secondary"
          className="w-full bg-surface-overlay hover:bg-surface-raised active:bg-surface-sunken border-surface-border text-text-primary shadow-sm"
