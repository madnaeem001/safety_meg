...
import { EMISSIONS_DATA } from '../../../data/mockRiskDigester';
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-8">
        <div>
          <h3 className="text-xl font-bold text-brand-900 tracking-tight">Environmental Emissions</h3>
          <p className="text-xs text-surface-500 font-medium uppercase tracking-widest mt-1">Actual vs Regulatory Limits</p>
        </div>
        <button 
          onClick={() => navigate('/emission-reports')}
          className="text-[10px] font-bold text-brand-600 uppercase tracking-widest hover:text-brand-700 transition-colors flex items-center gap-1"
        >
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={EMISSIONS_DATA}>
            ...
            <XAxis 
              ...
              axisLine={false} 
              tickLine={false} 
              tick={{ fill: '#a19379', fontSize: 10, fontWeight: 600 }}
              dy={10}
            />
            <YAxis 
              axisLine={false} 
