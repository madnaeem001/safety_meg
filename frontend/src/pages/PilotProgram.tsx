    ...
    <div className="page-wrapper">
      ...
      <div className="px-4 pt-20 pb-24 max-w-7xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-sm text-text-muted">30-Day Beta Program Manager</p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="flex gap-1 mb-6 overflow-x-auto pb-2 scrollbar-hide">
          {tabs.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${activeTab === tab ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'text-text-muted hover:text-text-primary hover:bg-surface-sunken'}`}>
              {tab}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'Pilot Dashboard' && (
          <div className="space-y-6">
            ...
            <div className="bg-surface-raised border border-surface-border rounded-2xl p-6">
              <h3 className="text-lg font-semibold text-text-primary mb-4">30-Day Pilot Progress</h3>
              <div className="space-y-4">
                {betaSites.filter(s => s.status === 'active').map(site => (
                  <div key={site.id} className="bg-surface-sunken rounded-xl p-4 border border-surface-border">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm text-text-muted">{site.daysLeft} days left</span>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-surface-raised border border-surface-border rounded-2xl p-6">
                <h3 className="text-lg font-semibold text-text-primary mb-3">Top UX Issues</h3>
                <div className="space-y-3">
                  {shadowingSessions.filter(s => s.severity === 'high').map(s => (
                    <div key={s.id} className="flex items-start gap-3 bg-red-500/10 border border-red-500/20 rounded-xl p-3">
                      ...
                      <div>
                        <p className="text-sm text-text-primary">{s.findings}</p>
                        <p className="text-xs text-text-muted mt-1">{s.site} • {s.observer}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-surface-raised border border-surface-border rounded-2xl p-6">
                <h3 className="text-lg font-semibold text-text-primary mb-3">Top Feature Requests</h3>
                <div className="space-y-3">
                  {feedbackItems.filter(f => f.type === 'idea').sort((a, b) => b.votes - a.votes).slice(0, 3).map(f => (
                    <div key={f.id} className="flex items-start gap-3 bg-cyan-500/10 border border-cyan-500/20 rounded-xl p-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-text-muted mt-1">{f.user} • {f.date}</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {activeTab === 'Beta Sites' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-lg font-semibold text-text-primary">Beta Deployment Sites</h3>
            ...
            {betaSites.map((site, i) => (
              <motion.div key={site.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="text-text-primary font-semibold text-lg">{site.name}</h4>
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${site.status === 'active' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-surface-sunken text-text-muted'}`}>
                        {site.status === 'active' ? '● Active' : '✓ Completed'}
                      </span>
                    </div>
                    <p className="text-sm text-text-muted">{site.department} • Started {site.startDate}</p>
                  </div>
                ...
                <div className="grid grid-cols-4 gap-4 mb-4">
                  <div className="text-center"><div className="text-xl font-bold text-text-primary">{site.enrolled}</div><div className="text-xs text-text-muted">Enrolled</div></div>
                  <div className="text-center"><div className="text-xl font-bold text-text-primary">{site.feedbackCount}</div><div className="text-xs text-text-muted">Feedback</div></div>
                  <div className="text-center"><div className="text-xl font-bold text-amber-400">{site.progress}%</div><div className="text-xs text-text-muted">Progress</div></div>
                  <div className="text-center"><div className="text-xl font-bold text-text-primary">{site.daysLeft}</div><div className="text-xs text-text-muted">Days Left</div></div>
                </div>
        ...
        {activeTab === 'Field Shadowing' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-lg font-semibold text-text-primary">Field Shadowing Sessions</h3>
            ...
            <p className="text-sm text-text-muted mb-4 bg-purple-500/10 border border-purple-500/20 rounded-xl p-3">
                ...
                <motion.div key={session.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-text-primary font-medium">{session.observer}</span>
                      <ChevronRight className="w-4 h-4 text-text-muted" />
                      <span className="text-text-secondary">observed</span>
                      <span className="text-text-primary font-medium">{session.worker}</span>
                    </div>
                  ...
                  <div className="bg-surface-sunken rounded-xl p-3 mb-2">
                    <p className="text-sm text-text-primary">{session.findings}</p>
                  </div>
        ...
        {activeTab === 'User Feedback' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-lg font-semibold text-text-primary">In-App Feedback Stream</h3>
              <div className="flex gap-2">
                {(['All', 'Bugs', 'Ideas', 'Praise'] as const).map(f => (
                  <button key={f} onClick={() => setFeedbackFilter(f)} className="px-3 py-1.5 rounded-lg text-xs font-medium text-text-muted hover:text-text-primary hover:bg-surface-sunken transition-all">{f}</button>
                ))}
              </div>
            </div>
            <div className="space-y-3">
                ...
                <motion.div key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <p className="text-text-primary mb-1">{item.message}</p>
                    ...
                    <button onClick={() => handleVote(item.id)} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-surface-sunken hover:bg-surface-overlay transition-all">
                      <ThumbsUp className="w-3.5 h-3.5 text-text-muted" />
                      <span className="text-sm text-text-primary font-medium">{item.votes}</span>
                    </button>
                  </div>
                </motion.div>
              ))}
        ...
        {activeTab === 'Pilot Report' && (
          <div className="space-y-6">
            <div className="bg-surface-raised border border-surface-border rounded-2xl p-6">
              <h3 className="text-lg font-semibold text-text-primary mb-4">30-Day Pilot Summary Report</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold text-emerald-400">{stats ? `${stats.avgAdoption}%` : '—'}</div><div className="text-xs text-text-muted mt-1">Adoption Rate</div>
                </div>
                <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold text-cyan-400">{stats ? stats.npsScore : '—'}</div><div className="text-xs text-text-muted mt-1">NPS Score</div>
                </div>
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold text-amber-400">{stats ? stats.uxIssuesFound : '—'}</div><div className="text-xs text-text-muted mt-1">UX Issues</div>
                </div>
                <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold text-purple-400">4.2</div><div className="text-xs text-text-muted mt-1">Avg Rating</div>
                </div>
              </div>
              <div className="space-y-3">
                <h4 className="text-text-primary font-medium">Go/No-Go Recommendation</h4>
                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <CheckCircle2 className="w-5 h-5 text-emerald-400" />
                    <span className="text-emerald-400 font-semibold text-lg">GO — Ready for Phase 2 Rollout</span>
                  </div>
                  <p className="text-sm text-text-secondary">High adoption rate (84%) with strong NPS (72). Critical UX issues (glove-friendly buttons, offline sync) are documented and scheduled for v1.1 fix. Recommend expanding to 3 additional sites with the identified improvements.</p>
                </div>
              </div>
            </div>
            <div className="bg-surface-raised border border-surface-border rounded-2xl p-6">
              <h3 className="text-text-primary font-semibold mb-3">Action Items for v1.1</h3>
              <div className="space-y-2">
                {['Increase all touch targets to 48px minimum for gloved operation', 'Add photo upload retry with offline queue', 'Implement voice-to-text for incident descriptions', 'Fix PDF export truncation for long descriptions', 'Add QR code linking for equipment-specific forms'].map((item, i) => (
                  <div key={i} className="flex items-center gap-3 bg-surface-sunken rounded-xl p-3">
