        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-300">Lead exposure in Paint Shop (EA-004) exceeds PEL by 24%. Immediate engineering controls and enhanced medical surveillance recommended per OSHA 29 CFR 1910.1025. 3 workers require blood lead level testing within 48 hours.</p>
            </div>
          </div>
        </motion.div>
          ...
          <div className="flex gap-2">
            {['all', 'chemical', 'physical', 'biological'].map(type => (
              <button key={type} onClick={() => setFilterType(type)}
                className={`px-3 py-2 rounded-xl text-xs font-medium capitalize transition-all ${filterType === type ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'bg-white/5 text-slate-400 border border-white/10 hover:bg-white/10'}`}>
                    ...
                    <div className="flex items-start gap-3 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <h3 className="text-sm font-bold text-white">{exp.agent}</h3>
                        ...
                        <div className="flex items-center gap-3 mt-1 flex-wrap">
                          <span className="text-xs text-slate-400 flex items-center gap-1"><MapPin className="w-3 h-3" /> {exp.location}</span>
                          <span className="text-xs text-slate-400 flex items-center gap-1"><Users className="w-3 h-3" /> {exp.workers} workers</span>
                          <span className="text-xs text-slate-400 flex items-center gap-1"><Calendar className="w-3 h-3" /> Next: {exp.nextDue}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 flex-shrink-0">
                      <div className="text-right">
                        <p className="text-xs text-slate-500">Measured / OEL</p>
                        <p className="text-sm font-bold text-white">{exp.measured} <span className="text-slate-500">/ {exp.oel}</span></p>
                      </div>
                ...
                <motion.div key={plan.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      <p className="text-sm font-bold text-white">{plan.dueDate}</p>
                    </div>
                  </div>
                </motion.div>
              ))}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {['OSHA PELs (29 CFR 1910.1000)', 'NIOSH RELs', 'ACGIH TLVs (2026)', 'AIHA WEELs'].map((std, i) => (
                    <div key={i} className="bg-white/5 rounded-xl p-3 border border-white/5">
                      <p className="text-xs text-slate-300 font-medium">{std}</p>
                    </div>
                  ))}
                </div>
              </div>
