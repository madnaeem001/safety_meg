      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 pt-20 md:pt-24 space-y-6">
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight">Observations & Coaching</h1>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-300">PPE non-compliance increased 18% in the Welding & Grinding areas this month. Recommend targeted toolbox talks and supervisor walk-throughs. Historical data suggests a correlation with shift changeover times (2PM-3PM).</p>
            </div>
          </div>
        </motion.div>
            ...
            <motion.div key="observations" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
              ...
              <div className="flex flex-col md:flex-row gap-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setFilterType('all')} className={`px-3 py-2 rounded-xl text-xs font-medium transition-all ${filterType === 'all' ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'bg-white/5 text-slate-400 border border-white/10'}`}>All</button>
                  <button onClick={() => setFilterType('safe')} className={`px-3 py-2 rounded-xl text-xs font-medium transition-all ${filterType === 'safe' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-white/5 text-slate-400 border border-white/10'}`}>Safe</button>
                  <button onClick={() => setFilterType('at_risk')} className={`px-3 py-2 rounded-xl text-xs font-medium transition-all ${filterType === 'at_risk' ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-white/5 text-slate-400 border border-white/10'}`}>At Risk</button>
                </div>
              </div>
                  ...
                  <div className="flex flex-col gap-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-300 mt-1">{obs.behavior}</p>
                    ...
                    {obs.feedback && (
                      <div className="ml-13 pl-4 border-l-2 border-white/10">
                        <p className="text-xs text-slate-400"><span className="text-cyan-400 font-medium">Coaching Note:</span> {obs.feedback}</p>
                      </div>
                    )}
                  </div>
                </motion.div>
                  ...
                  {[
                    ...
                    <div key={i} className="bg-white/5 rounded-xl p-4 border border-white/5">
                      <div className="flex items-center gap-3 mb-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${i === 0 ? 'bg-amber-500/20 text-amber-400' : i === 1 ? 'bg-slate-400/20 text-slate-300' : 'bg-orange-500/20 text-orange-400'}`}>
                          #{champ.rank}
                        </div>
                        <div>
                          <p className="text-sm font-bold text-white">{champ.name}</p>
                          <p className="text-[10px] text-slate-400">Safety Champion</p>
                        </div>
                      </div>
                      <div className="grid grid-cols-3 gap-2 text-center">
                        <div><p className="text-lg font-bold text-white">{champ.obs}</p><p className="text-[9px] text-slate-500 uppercase">Obs</p></div>
                        <div><p className="text-lg font-bold text-white">{champ.coached}</p><p className="text-[9px] text-slate-500 uppercase">Coached</p></div>
                        <div><p className="text-lg font-bold text-cyan-400">{champ.rate}</p><p className="text-[9px] text-slate-500 uppercase">Rate</p></div>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  <div key={i} className="py-3 border-b border-white/5 last:border-0">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">{obs.feedback}</p>
                        <p className="text-[10px] text-slate-500 mt-1">Coach: {obs.observer} • {obs.date}</p>
                      </div>
